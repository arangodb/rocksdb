...
VersionStorageInfo::VersionStorageInfo(
    ...
    : internal_comparator_(internal_comparator),
      ...
      estimated_compaction_needed_bytes_level_(num_levels_),
      finalized_(false),
...
void VersionStorageInfo::EstimateCompactionBytesNeeded(
  ...
  if (static_cast<int>(files_[0].size()) >=
    ...
    estimated_compaction_needed_bytes_level_[0] = level_size;
    bytes_compact_to_next_level = level_size;
  } else {
    estimated_compaction_needed_bytes_ = 0;
    estimated_compaction_needed_bytes_level_[0] = 0;
  }
  ...
  for (int level = base_level(); level <= MaxInputLevel(); level++) {
    level_size = 0;
    estimated_compaction_needed_bytes_level_[level] = 0;
    ...
    if (level == base_level() && level0_compact_triggered) {
      ...
      estimated_compaction_needed_bytes_ += level_size;
      estimated_compaction_needed_bytes_level_[level] += level_size;
    }
      ...
      if (bytes_next_level > 0) {
        ...
        auto pending = static_cast<uint64_t>(
            ...
            (static_cast<double>(bytes_next_level) /
             ...
             1));
        estimated_compaction_needed_bytes_ += pending;
        estimated_compaction_needed_bytes_level_[level] += pending;
      }
    }
  }
}
